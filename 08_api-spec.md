...
- 개정일: 2026-09-09 / 승인된 목표 계약 및 블록체인 P2 조건부 상태 정리
...
- 현재 계약은 일반 HTTPS·인증·데이터베이스 기반 API를 전제로 합니다. 블록체인·분산원장·스마트계약·QR 검증 API는 포함하지 않습니다. [데이터 명세의 P2 도입 게이트](04_data-spec.md#11-6-도입-게이트)를 모두 충족하고 PoC가 별도 승인된 뒤에만 발급·등록·검증·장애 상태 계약을 추가하며, 그전에는 실제 호출 경로나 적용 예정 API로 설명하지 않습니다.
