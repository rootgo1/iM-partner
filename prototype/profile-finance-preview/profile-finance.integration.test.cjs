...
assert.match(ui, /나의 가게 온도/);
assert.match(ui, /연결 데이터 /);
assert.match(ui, /선택 기간 지출 ÷ 매출 × 100/);
assert.match(component, /label: '나의 가게 온도'/);
...
assert.match(component, /element\('strong', '', '나의 가게 온도'\)/);
assert.doesNotMatch(component, /element\('span', 'ipf-profile-finance__basis'/);
