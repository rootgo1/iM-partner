...
- 개정일: 2026-09-09 / 목표 논리 스키마 및 블록체인 P2 조건부 상태 정리
...
- 현재 이력·변경 추적 설계는 MySQL의 키·제약조건·출처·버전·감사 필드를 사용합니다. 블록체인·분산원장·스마트계약 테이블은 현재 범위에 추가하지 않습니다. [데이터 명세의 P2 도입 게이트](04_data-spec.md#11-6-도입-게이트)를 모두 충족하고 PoC가 별도 승인된 뒤에만 후보 필드를 실제 스키마로 설계하며, 그전에는 현행 테이블 목록이나 마이그레이션 대상으로 보지 않습니다.
